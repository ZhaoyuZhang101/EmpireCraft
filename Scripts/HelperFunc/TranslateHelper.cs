...
namespace EmpireCraft.Scripts.HelperFunc
{
    public static class TranslateHelper
    {
        ...
        public static void LogLeaveTakenAlliance(Kingdom kingdom, Empire empire)
        {
            ...
            new WorldLogMessage(EmpireCraftWorldLogLibrary.退出朝贡国,
                kingdom.name,
                empire.name)
            {
                color_special1 = kingdom.getColor()._color_text,
                color_special2 = empire.CoreKingdom != null ? empire.CoreKingdom.getColor()._color_text : kingdom.getColor()._color_text

            }.RecordIntoEmpire(empire);
        }

        public static void LogDecisionBlocked(Kingdom kingdom, string decisionName, string reason, Empire empire = null)
        {
            if (kingdom == null)
            {
                return;
            ...
            new WorldLogMessage(EmpireCraftWorldLogLibrary.帝国决议受阻,
                kingdom.name,
                decisionName ?? "",
                reason ?? "")
            {
            ...
            }.RecordIntoEmpire(empire ?? kingdom.GetEmpire());
        }

        public static void LogLeaveEmpire(Kingdom kingdom, Empire empire)
        {
            if (kingdom == null || empire == null)
            {
                return;
            ...
            new WorldLogMessage(EmpireCraftWorldLogLibrary.脱离帝国,
                kingdom.name,
                empire.name)
            {
                color_special1 = kingdom.getColor()._color_text,
        ...
        public static void LogEmpireAbandonRebellion(Empire empire, Kingdom rebelKingdom)
        {
            if (empire == null || rebelKingdom == null)
            {
                return;
            }

            new WorldLogMessage(EmpireCraftWorldLogLibrary.帝国放弃平叛,
                empire.name,
                rebelKingdom.name)
            {
                color_special1 = empire.getColor()._color_text,
                color_special2 = rebelKingdom.getColor()._color_text
