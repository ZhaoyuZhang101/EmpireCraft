...
public static class GameAIMain
{
    ...
    public static void KingdomAIs(this BehaviourTaskKingdom t, ModBehaviourTaskKingdomLibrary lib)
    {
        ...
        foreach (var type in types)
        {
            ...
            foreach (var action in lib.list.ToList().SelectMany(bt => bt.list.ToList()))
            {
                if (action.id == id)
                {
                    LogService.LogInfo($"存在原版同类逻辑{id}，已覆盖");
                    t.list.Remove(action);
                }
            }
    ...
    public static void CityAIs(this BehaviourTaskCity t, ModBehaviourTaskCityLibrary lib)
    {
        var asm = Assembly.GetExecutingAssembly();
        var types = asm.GetTypes()
            .Where(ty =>
    ...
    public static void ActorAIs(this BehaviourTaskActor t, ModBehaviourTaskActorLibrary lib)
    {
        var asm = Assembly.GetExecutingAssembly();
        var types = asm.GetTypes()
            .Where(ty =>
